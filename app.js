...
{min:0,max:1,color:"rgba(72,149,239,.16)"},{min:1,max:1.5,color:"rgba(82,183,136,.16)"},{min:1.5,max:2.5,color:"rgba(248,196,62,.18)"},{min:2.5,max:3.5,color:"rgba(244,140,54,.18)"},{min:3.5,max:4.5,color:"rgba(214,69,65,.18)"},{min:4.5,max:Math.max(6,y.max),color:"rgba(83,52,131,.18)"}];ctx.save();bands.forEach(b=>{const top=y.getPixelForValue(Math.min(b.max,y.max));const bottom=y.getPixelForValue(Math.max(b.min,y.min));ctx.fillStyle=b.color;ctx.fillRect(chartArea.left,top,chartArea.right-chartArea.left,bottom-top)});ctx.restore()}};
...
async function loadLocation(lat,lon,label){
 ...
 try{const r=await fetchWeather(lat,lon);state.hourly=r.hourly;state.daily=r.daily;state.label=label;locationTitle.textContent=label;locationDetails.textContent=`Latitude ${fmt(lat,4)} · longitude ${fmt(lon,4)} · altitude modèle ${fmt(r.elevation,0)} m`;locationMessage.textContent="Données actualisées.";renderAll()}
...
function renderDashboard(){
 ...
 const windows=next48.map(x=>({...x,q:irrigationQuality(x)})).sort((a,b)=>b.q.score-a.q.score);const best=windows[0];
 bestWindow.textContent=dateLabel(best.time);bestWindowDetails.textContent=`${best.q.label} · DPV ${fmt(best.vpd)} kPa · vent ${fmt(best.wind)} km/h`;
 const deficit=computeDeficit(pastDays(30));dashboardDeficit.textContent=`${fmt(deficit.at(-1)?.cumulative||0)} mm`;

 destroy("dashScore");state.charts.dashScore=new Chart(dashboardScoreChart,{type:"bar",data:{labels:fd.map(d=>dayLabel(d.date)),datasets:[{label:"Indice /100",data:fd.map(d=>d.score),backgroundColor:fd.map(d=>d.score<25?"rgba(72,149,239,.75)":d.score<50?"rgba(82,183,136,.75)":d.score<75?"rgba(248,196,62,.8)":d.score<90?"rgba(244,140,54,.8)":"rgba(83,52,131,.8)"),borderRadius:6}]},options:{...chartBase,scales:{x:chartBase.scales.x,y:{beginAtZero:true,max:100}}}});
...
function renderVpd(){
 ...
 const load=rows.reduce((s,x)=>s+Math.max(0,x.vpd-1.5),0);vpdLoad.textContent=`${fmt(load)} kPa·h`;
 vpdReading.innerHTML=`<div class="reading-item"><strong>0–1,0 kPa</strong>Faible demande atmosphérique</div><div class="reading-item"><strong>1,0–1,5 kPa</strong>Conditions favorables</div><div class="reading-item"><strong>1,5–2,5 kPa</strong>Début de régulation stomatique</div><div class="reading-item"><strong>2,5–3,5 kPa</strong>Contrainte atmosphérique élevée</div><div class="reading-item"><strong>3,5–4,5 kPa</strong>Stress sévère</div><div class="reading-item"><strong>> 4,5 kPa</strong>Stress extrême</div>`
}

function renderScore(){
 const fd=forecastDays().map(scoreDay);
 destroy("score");state.charts.score=new Chart(scoreChart,{type:"bar",data:{labels:fd.map(d=>dayLabel(d.date)),datasets:[{label:"Indice /100",data:fd.map(d=>d.score),backgroundColor:fd.map(d=>d.score<25?"rgba(72,149,239,.75)":d.score<50?"rgba(82,183,136,.75)":d.score<75?"rgba(248,196,62,.8)":d.score<90?"rgba(244,140,54,.8)":"rgba(83,52,131,.8)"),borderRadius:6}]},options:{...chartBase,scales:{x:chartBase.scales.x,y:{beginAtZero:true,max:100}}}});
 scoreCards.innerHTML=fd.map(d=>`<article class="forecast-card"><span>${dayLabel(d.date)}</span><strong>${d.score}/100</strong><span>${scoreLabel(d.score)}</span><span>DPV max ${fmt(d.maxVpd)} kPa</span><span>${d.hours25} h > 2,5 kPa</span><span>Tmax ${fmt(d.tmax)} °C</span><span>ET₀ ${fmt(d.et0)} mm</span></article>`).join("")
}

function renderIrrigation(){
 const rows=futureHours(48).map(x=>({...x,q:irrigationQuality(x)})).sort((a,b)=>b.q.score-a.q.score).slice(0,12).sort((a,b)=>new Date(a.time)-new Date(b.time));
 irrigationTable.innerHTML=rows.map(x=>`<tr><td>${dateLabel(x.time)}</td><td><span class="badge ${x.q.class}">${x.q.label}</span></td><td>${fmt(x.vpd)} kPa</td><td>${fmt(x.wind)} km/h</td><td>${fmt(x.temperature)} °C</td><td>${fmt(x.rain)} mm</td></tr>`).join("")
}

function renderOthers(){
 const fd=forecastDays(),future=futureHours(168);
 hours35.textContent=`${future.filter(x=>x.temperature>35).length} h`;tropicalNights.textContent=`${fd.filter(d=>d.tmin>=20).length} nuit(s)`;photoHours.textContent=`${future.filter(x=>x.temperature>=18&&x.temperature<=30&&x.vpd>=1&&x.vpd<=1.5&&x.radiation>100).length} h`;wetHours.textContent=`${future.filter(x=>x.rain>0||x.humidity>=90).length} h`;
 const byDay=fd.map(d=>{const h=hourlyForDay(d.date);return{date:d.date,heat:h.filter(x=>x.temperature>35).length,wet:h.filter(x=>x.rain>0||x.humidity>=90).length}});
 destroy("heat");state.charts.heat=new Chart(heatChart,{type:"bar",data:{labels:byDay.map(d=>dayLabel(d.date)),datasets:[{label:"Heures > 35 °C",data:byDay.map(d=>d.heat),backgroundColor:"rgba(214,69,65,.78)",borderRadius:5}]},options:chartBase});
 destroy("wet");state.charts.wet=new Chart(wetnessChart,{type:"bar",data:{labels:byDay.map(d=>dayLabel(d.date)),datasets:[{label:"Heures humides estimées",data:byDay.map(d=>d.wet),backgroundColor:"rgba(72,149,239,.78)",borderRadius:5}]},options:chartBase})
}
